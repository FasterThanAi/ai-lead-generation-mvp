...
function EmailExtraction({ campaignId, onExtractionComplete }) {
  ...
  const applyCampaignStatus = useEffectEvent((status) => {
      ...
      try {
        ...
        setError(
          getFriendlyErrorMessage(
            err,
            "Could not load email extraction progress."
          )
        );
        console.error(err);
      }
  ...
  useEffect(() => {
    ...
    getCampaignExtractionStatus(campaignId)
      .then((status) => {
        if (!ignore) {
          applyCampaignStatus(status);
        }
      })
      .catch((err) => {
        if (!ignore) {
  ...
  const handleExtractCampaignEmails = async () => {
    ...
    try {
      ...
      if (result.status === "nothing_to_do") {
        setJob(null);
        setSummary(
          "No leads need email extraction. Leads either already have emails or are missing websites."
        );
        return;
      }
      ...
      setError(
        err.response
          ? detail || "Email extraction failed. Please try again."
    ...
    <Card>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          ...
          <div className="rounded-lg border border-warn-soft bg-warn-soft p-3">
            ...
            <p className="mt-1 font-semibold text-warn">
              {campaignStatus.eligible_without_email}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-info-soft bg-info-soft p-3">
            ...
            <p className="mt-1 font-semibold text-info">
              {campaignStatus.coverage_percent}%
            </p>
          </div>
        </div>
      )}
      ...
      {isJobRunning && (
        <div className="mb-4 rounded-lg border border-info-soft bg-info-soft p-3">
          <div className="mb-2 flex items-center justify-between gap-3 text-xs text-info">
            <span>
              Processing {job.processed ?? 0}/{job.total ?? 0} leads
            </span>
            <span>{progress}%</span>
          </div>
      ...
      {campaignId && (
        <p className="mt-3 text-xs text-muted">
          Runs in the background and checks progress every 3 seconds, so large
          campaigns avoid Render request timeouts.
        </p>
      )}
    </Card>
  );
